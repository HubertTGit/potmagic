...
export const muteAllViewers = createServerFn({ method: 'POST' })
  .inputValidator((input) =>
    z.object({ storyId: z.string(), muted: z.boolean() }).parse(input),
  )
  .handler(async ({ data }) => {
    const session = await getSessionOrThrow();
    ...
    if (!story || story.directorId !== session.user.id)
    ...
    const participants = await svc.listParticipants(data.storyId);
    const viewers = participants.filter((p) =>
      p.identity.startsWith(VIEWER_PREFIX),
    );
    ...
    return { mutedCount: viewers.length };
  });
