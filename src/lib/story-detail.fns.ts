...
export const getStoryDetail = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const directorOnStage =
      story.status === "draft" || story.status === "active"
        ? await isDirectorOnStage(story.id, story.directorId)
        : false;

    return { story: { ...story, directorOnStage }, scenes: sceneRows };
  });
...
export const updateStoryTitle = createServerFn({ method: "POST" })
  .inputValidator((input) =>
    z
      .object({ storyId: z.string(), title: z.string().min(1).max(200) })
      .parse(input),
  )
  .handler(async ({ data }) => {
    await requireStoryOwner(data.storyId);
...
export const addScene = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [{ maxOrder }] = await db
      .select({
        maxOrder: sql<number>`cast(coalesce(max("order"), 0) as integer)`,
      })
      .from(scenes)
      .where(eq(scenes.storyId, data.storyId));
...
export const updateSelectedScene = createServerFn({ method: "POST" })
  .inputValidator((input) =>
  ...
  .handler(async ({ data }) => {
    ...
    await db
      .update(stories)
      .set({ selectedSceneId: data.sceneId })
      .where(eq(stories.id, data.storyId));
  });
